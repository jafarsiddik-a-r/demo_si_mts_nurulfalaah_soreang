...
@php
    ...
    $baseClass = "w-full bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 border-2 rounded p-2 sm:px-3 sm:py-1.5 text-base focus:outline-none transition-colors disabled:bg-gray-100 disabled:text-gray-500 dark:disabled:bg-slate-800 dark:disabled:text-slate-500 {$errorClass} {$class}";
@endphp
...
<div class="w-full">
    ...
    <div class="relative">
        ...
        @if($type === 'password')
             <button type="button" data-toggle-password-for="{{ $id }}" class="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300">
